...
def find_typerefs(in_file, node, n):
    ...
    if node.kind == CursorKind.USING_DIRECTIVE:
        ...
        t1 = tokens[0]
        t2 = tokens[-1]
        add2(t1.extent.end.line, t1.extent.start.column, t2.extent.end.column-1, "", t1.spelling)
    ...
    if node.kind == CursorKind.NAMESPACE:
        for t in node.get_tokens():
            if t.cursor.kind == CursorKind.NAMESPACE and (t.location.line == node.extent.start.line or t.location.line == node.extent.end.line):
                add2(t.extent.end.line, t.extent.start.column, t.extent.end.column-1, "", t.spelling)



    if (node.kind == CursorKind.FUNCTION_DECL)  and len(new_name) > 0:
        add(node.location.line, node.location.column, node.location.column + len(node.spelling) -1, new_name, node.spelling)
...
#                    print 'tokens:', c.spelling, t2.spelling
                    add2(arg.location.line, t.extent.start.column, t2.extent.end.column -1, '', str([t.spelling for t in tokens]))
                    break
            ...
            for t in tokens:
                if t.spelling == node.spelling:
                    if t.location.line == node.location.line:
                        add2(t.location.line, t.location.column, t.location.column + len(t.spelling) -1, new_name, t.spelling)
                    else:
            ...
            if len(tokens)>=2:
                ...
                if t1.spelling == node.spelling:
                    if t1.location.line == node.location.line:
                        add(t1.extent.start.line, t1.extent.start.column, t2.extent.end.column-1, "", node.spelling)
                    else:
                        if(rdebug):  print debug.s(n), "macro found"
...
#        print default_args
        ...
        tokens = reversed(list(node.get_tokens())[:-1])
        for token in tokens:
            if token.spelling == ')':
                break;
        comma = ''
...
#                print new_arg
                add2(token.extent.start.line, token.extent.end.column-1, token.extent.end.column-2, comma + str(new_arg), "")
