...
const PROGMEM struct cprintf::ArrayData batteryTypeData = CPRINTF_STRING_ARRAY_DATA(batteryString, battery.type);


#define Tmin    (SETTINGS_FAN_TEMPERATURE_DIFF/ANALOG_CELCIUS(1))*ANALOG_CELCIUS(1) + ANALOG_CELCIUS(1)
#define Tmax    ANALOG_CELCIUS(99)
#define Tstep   ANALOG_CELCIUS(1)

#ifdef SDCC_COMPILER
#define BATTERY_N(type, n, x)   {CP_TYPE_ ## type, n, {.uint16Ptr = (uint16_ptr) &ProgramData_battery.x}}
#else
#define BATTERY_N(type, n, x)   {CP_TYPE_ ## type, n, {.uint16Ptr =(uint16_ptr) &battery.x}}
#endif

#define BATTERY(type, x)        BATTERY_N(type, 0, x)
...
{string_batteryVolt,    COND_BATT_UNKN,     CPRINTF_METHOD(Screen::StartInfo::printVoltageString),
#ifndef SDCC_COMPILER //TODO:
        STATIC_EDIT_METHOD(changeVoltage)
#endif
},
