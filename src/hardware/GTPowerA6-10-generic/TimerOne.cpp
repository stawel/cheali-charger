...
void TimerOne::pwm(char pin, unsigned int duty, long microseconds)  // expects duty cycle to be 10 bit (1024)
{
  ...
  if(pin == 1 || pin == 9 || pin == 14) {
    TCCR1A |= _BV(COM1A1);                                 // activates the output pin
  }
  else if(pin == 2 || pin == 10 || pin == 13) {
    TCCR1A |= _BV(COM1B1);
  }
  setPwmDuty(pin, duty);
...
{
  if(pin == 1 || pin == 9 || pin == 14)       TCCR1A &= ~_BV(COM1A1);   // clear the bit that enables pwm on PB1
  else if(pin == 2 || pin == 10 || pin == 13) TCCR1A &= ~_BV(COM1B1);   // clear the bit that enables pwm on PB2
}
